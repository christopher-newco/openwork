  ...
  if (diskSizeGB > 0) {
    try {
      ...
      const updatedDockerCommand = [
        "/bin/sh",
        "-c",
        "set -x && mkdir -p /workspace && exec openwork serve --workspace /workspace --remote-access --openwork-port ${PORT:-10000} --openwork-host 0.0.0.0 --opencode-host 127.0.0.1 --opencode-port 4096 --connect-host 0.0.0.0 --cors '*' --approval manual --allow-external --opencode-source external --no-opencode-router --verbose",
      ]
